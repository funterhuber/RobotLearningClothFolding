...
class DiffusionPolicy(PreTrainedPolicy):
    ...
    def get_optim_params(self) -> dict:
        if self.config.vision_backbone_lr is not None and self.config.train_vision_backbone:
            backbone_params = []
            other_params = []
            for name, param in self.diffusion.named_parameters():
                if not param.requires_grad:
                    continue
                if "rgb_encoder" in name and "backbone" in name:
                    backbone_params.append(param)
                else:
                    other_params.append(param)
            return [
                {"params": backbone_params, "lr": self.config.vision_backbone_lr},
                {"params": other_params}
            ]
        else:
...
class DiffusionRgbEncoder(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        if config.vision_backbone.startswith("dinov3"):
            class Dinov3Wrapper(nn.Module):
                ...
                def forward(self, x):
                    ...
                    return patch_tokens.reshape(B, H_out, W_out, C).permute(0, 3, 1, 2)

            self.backbone = Dinov3Wrapper(config.vision_backbone)
        else:
        ...
        if getattr(config, "train_vision_backbone", True) is False:
